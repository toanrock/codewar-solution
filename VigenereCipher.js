...
function VigenèreCipher(key, abc) {
    ...
    this.encode = function (str) {
        ...
        // match the key length with the plain tex
        let newkey = key
        ...
        for (let i = 0; i < str.length; i++) {
            ...
            if (abc.indexOf(str[i]) === -1) {
                ...
                let indexshift = abc.indexOf(str[i]) + abc.indexOf(key[i])
                if (indexshift >= abc.length) { indexshift -= abc.length }
                result += abc[indexshift]
    ...
    this.decode = function (str) {
        //...
        let result = ""
        // match the key length with the cypher tex
        let newkey = key
        while (newkey.length < str.length) {
            newkey += key
        }
        ...
        for (let i = 0; i < str.length; i++) {
            ...
            if (abc.indexOf(str[i]) === -1) {
                ...
                let indexAbc = abc.indexOf(str[i]) - abc.indexOf(key[i])
                if (indexAbc < 0) { indexAbc += abc.length }
                result += abc[indexAbc]
            }
        }
        return result
